...
\subsubsection{Características del Sistema de Colas}
...
\subsubsection{Software Requerido}
\begin{itemize}
    \item Compilador C++ compatible con C++11 o superior
...
\subsubsection{Variables Globales Principales}
\paragraph{Parámetros del Sistema}
...
\subsubsection{Archivo de Entrada (\texttt{param.txt})}
\begin{lstlisting}
5.0 6.0 1000 1 3
\end{lstlisting}
...
\subsubsection{Verificación de Resultados Teóricos}
...
\subsubsection{Comando de Compilación}
\begin{lstlisting}[language=bash]
g++ "Sistema de Colas.cpp" -o SistemaDeColas
\end{lstlisting}
...
El simulador de colas M/M/1 desarrollado en C++ ha demostrado ser una herramienta efectiva para modelar y analizar sistemas de colas simples. A través de la implementación de un modelo de colas M/M/1, se ha logrado simular el comportamiento de un sistema con llegadas y servicios exponenciales, permitiendo observar métricas clave como el tiempo promedio en el sistema, la utilización del servidor y el tamaño promedio de la cola.

La restructuración del código siguiendo el modelo SAO (Sistema-Ambiente-Observatorio) y la implementación de los seis módulos fundamentales propuestos por Ortiz T. resultó en una arquitectura robusta y extensible. Esta modularización facilitó la evolución progresiva desde el modelo base M/M/1 hasta las implementaciones más complejas, demostrando la importancia del diseño arquitectónico en el desarrollo de simuladores escalables.
El simulador M/M/m (Erlang B y C) ha permitido explorar configuraciones con múltiples servidores, proporcionando una comprensión más profunda de cómo la capacidad del sistema afecta las métricas de rendimiento. Los resultados obtenidos en los experimentos con Erlang B y C validan la teoría de colas y muestran cómo la utilización del sistema y la probabilidad de bloqueo cambian con el número de servidores y la tasa de llegada.
Los experimentos realizados confirman que las implementaciones de los modelos M/M/1, M/M/m (Erlang B y C), y Geo/Geo/m/N reproducen adecuadamente el comportamiento teórico de los sistemas de colas estudiados. La convergencia sistemática hacia los valores teóricos, especialmente evidente en el modelo Geo/Geo/m/N con simulaciones extensas (2 millones de slots), valida tanto la correctitud de las implementaciones como la aplicabilidad práctica de estos modelos.

La experimentación demostró que el tamaño muestral es crítico para la precisión de las estimaciones, particularmente para eventos raros como las probabilidades de bloqueo. En el modelo Geo/Geo/m/N, el error relativo en la probabilidad de bloqueo disminuyó dramáticamente desde 78.57\% (100K slots) hasta 0.64\% (2M slots), siguiendo el comportamiento esperado según la Ley de los Grandes Números. Este resultado establece guías prácticas para seleccionar duraciones de simulación apropiadas según el nivel de precisión requerido.

Los resultados obtenidos proporcionan herramientas validadas para el análisis de sistemas de colas en aplicaciones prácticas. Los simuladores desarrollados permiten evaluar el impacto de diferentes configuraciones de servidores, tasas de llegada y capacidades del sistema, facilitando la toma de decisiones informadas en el diseño de sistemas de telecomunicaciones, centros de servicio y redes de computadoras. La implementación del modelo discreto Geo/Geo/m/N es particularmente valiosa para el análisis de sistemas digitales donde los eventos ocurren en intervalos de tiempo fijos.
\section{Referencias}
