...
Al finalizar cada simulación, se exportan datos relevantes a archivos de salida, incluyendo:

\begin{itemize}
    \item Posiciones de los nodos
    \item Roles asignados (líder o seguidor)
    \item Trayectorias seguidas
    \item Ubicación de eventos externos (e.g., incendios)
\end{itemize}
...
La clase \texttt{BoidsMobilityModel} incluye métodos \texttt{Set} que permiten ajustar los parámetros más relevantes:

\begin{itemize}
    \item Radio de cohesión de los seguidores
    \item Radio de influencia de los líderes
    \item Velocidad máxima de los nodos
    \item Frecuencia de actualización de posición
    \item Inicialización del rol de los nodos (porcentaje de líderes)
    \item Parámetros del WCA (pesos, umbrales de energía, distancia, etc.)
\end{itemize}

Esta configurabilidad permite experimentar con múltiples condiciones y validar el comportamiento del sistema bajo distintos escenarios.

\subsection{Simulación de escenarios}

Con el objetivo de evaluar el impacto del liderazgo dinámico en la efectividad de la red, se diseñaron y ejecutaron tres escenarios distintos. Cada uno varía condiciones como el número de eventos externos, la densidad de nodos y la velocidad promedio. Se analizó principalmente:

\begin{itemize}
    \item El tiempo promedio de extinción de incendios
    \item La cobertura efectiva del área de eventos
    \item El número total de eventos gestionados con éxito
\end{itemize}

Estos escenarios permitieron medir el rendimiento y adaptabilidad del modelo bajo condiciones realistas y dinámicas.
